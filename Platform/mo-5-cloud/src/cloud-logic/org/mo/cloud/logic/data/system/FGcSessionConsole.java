         ...
         IGcSessionConsole
{
   ...
   // ============================================================
   @Override
   public boolean verifyURL(TreeSet<String> params,
                            String clientSign,
                            long appId,
                            ILogicContext logicContext,
                            TDateTime appDate){
      StringBuilder sb = new StringBuilder();
      params.add("format_cd=json");
      for(String string : params){
         sb.append(string + "&");
      }
      FDataSystemApplicationUnit unit = logicContext.findLogic(FDataSystemApplicationLogic.class).find(appId);
      if(unit != null){
         sb.append("app_key=" + unit.guid());
         String serverSign = RMd5.encode(sb.toString());
         clientSign = clientSign.toUpperCase();
         _logger.debug(this, "verifyURL", "verifyURL begin serverSign={1},clientSign={2},requestParams={3}", serverSign, clientSign, sb.toString());
         if(clientSign.equals(serverSign)){
            TDateTime start = RDateTime.currentDateTime();
            start.addMinute(-10);
            start.addSecond(-8);
            TDateTime end = RDateTime.currentDateTime();
            end.addMinute(1);
            _logger.debug(this, "verifyURL", "verifyURL finish.appDate={1},start={2},end={3}", appDate.format("yyyy/mm/dd/ hh24:mi:ss:ms"), start.format("yyyy/mm/dd/ hh24:mi:ss:ms"), end.format("yyyy/mm/dd/ hh24:mi:ss:ms"));
            if(appDate.isBetween(start, end)){
               return true;
            }
         }
      }
