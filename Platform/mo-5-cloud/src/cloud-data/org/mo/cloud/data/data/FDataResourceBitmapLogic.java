      ...
      extends FLogicTable
{
   ...
   // 字段协议代码的定义。
   public final static SLogicFieldInfo MIME_CODE = new SLogicFieldInfo("MIME_CODE");
   ...
   // 字段集合的定义。
   public final static String FIELDS = "`OUID`,`OVLD`,`GUID`,`USER_ID`,`PROJECT_ID`,`RESOURCE_ID`,`CODE`,`LABEL`,`FORMAT_CODE`,`MIME_CODE`,`SIZE_WIDTH`,`SIZE_HEIGHT`,`NOTE`,`CREATE_USER_ID`,`CREATE_DATE`,`UPDATE_USER_ID`,`UPDATE_DATE`";
   ...
   protected <T extends FLogicUnit> T innerCreateUnit(){
      return (T)new FDataResourceBitmapUnit();
   }
   ...
   protected <T extends FLogicUnit> FLogicDataset<T> innerCreateDataset(){
      return (FLogicDataset<T>)new FLogicDataset<FDataResourceBitmapUnit>(FDataResourceBitmapUnit.class, _logicContext);
   }
   ...
   public EResult doInsert(FLogicUnit logicUnit){
      ...
      cmd.append(',');
      String mimeCode = unit.mimeCode();
      if(RString.isEmpty(mimeCode)){
         cmd.append("NULL");
      }else{
         cmd.append('\'');
         cmd.append(RSql.formatValue(mimeCode));
         cmd.append('\'');
      }
      cmd.append(',');
