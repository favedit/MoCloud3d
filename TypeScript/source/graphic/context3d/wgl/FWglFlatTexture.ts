module sk.graphic.context3d.wgl {
   ...
   // @history 141230
   //==========================================================
   export class FWglFlatTexture extends sk.graphic.context3d.FG3dFlatTexture {
      // @attribute
      _handle = null;
      _statusUpdate = false;
      ...
      public uploadData(content, width, height) {
         ...
         if (content.constructor == ArrayBuffer) {
            ...
            if (!context.enableFloatTexture()) {
               throw new sk.common.lang.FError(this, 'Invalid content float format.');
            }
            data = content;
         } else {
            throw new sk.common.lang.FError(this, 'Invalid content format.');
         }
         // 设置属性
         // o.width = width;
         // o.height = height;
      ...
      public upload(content, left, top, width, height) {
         ...
         if ((tagName == 'IMG') || (tagName == 'VIDEO') || (tagName == 'CANVAS')) {
            ...
            throw new sk.common.lang.FError(this, 'Invalid image format.');
         }
         // 绑定数据
         handle.bindTexture(handle.TEXTURE_2D, o._handle);
         // 设置上下反转
