module sk.common.xml {
   ...
   export class RXml {
      ...
      public static isNode(n) {
         return sk.common.reflect.RClass.isName(n, 'TNode');
      }
      ...
      public static buildNode(pd, pn, pe) {
         ...
         if (eas) {
            var eac = eas.length;
            if (eac > 0) {
               xas = new sk.common.lang.FAttributes();
               for (var n = 0; n < eac; n++) {
                  var ea = eas[n];
                  if (ea.nodeName) {
         ...
         // 建立文本
         var xt = new sk.common.lang.FString();
         xt.append(pe.value);
         ...
         // 创建节点
         var xc = pd.create(pe.nodeName, xas, sk.common.lang.RString.trim(xt.toString()));
         if (pn) {
            pn.push(xc);
         } else {
            pd._root = xc;
      ...
      public static unpack(s, n) {
         var o = this;
         if (sk.common.lang.RString.isEmpty(s)) {
            return null;
         }
         if (!n) {
            n = new FNode();
         }
         var np = new sk.common.lang.FAttributes();
         np.unpack(s);
         n.name = np.get('name');
         ...
         if (np.contains('nodes')) {
            //var ns = new sk.common.lang.FStrings();
            ...
            if (value != null) {
               ...
               switch (typeName) {
                  ...
                  case 'string':
                     ...
                     break;
                  case 'function':
                     xtag.setValue(sk.common.reflect.RMethod.name(value));
                     break;
                  case 'object':
                     o.saveObject(xtag, 'Property', value);
                     break;
                  default:
                     throw new sk.common.lang.FError(this, 'Invalid object.');
               }
            }
         }
      }
